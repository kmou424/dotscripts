...
# Set Date 
DATE=$(export TZ=Asia/Shanghai; date +"%Y%m%d-%T")
		...
		then
			...
			then
				...
				python2 "$KERNEL_DIR/scripts/ufdt/libufdt/utils/src/mkdtboimg.py" \
					create "$KERNEL_DIR/out/arch/arm64/boot/dtbo.img" "$KERNEL_DIR/out/arch/arm64/boot/dts/qcom/*.dtbo"
			fi
				gen_zip
			else
			if [ "$PTTG" = 1 ]
